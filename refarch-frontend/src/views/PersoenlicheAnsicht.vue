  ...
  <v-container>
    <v-row>
      <v-col cols="12">
        <h2 class="d-flex justify-between align-center">
          ...
          <v-btn class="ml-auto" @click="openModal">ToDo hinzufügen</v-btn>
        </h2>
      </v-col>
    </v-row>
    <v-row>
    ...
    <v-dialog v-model="modal" max-width="600px">
      <v-card>
        ...
        <v-card-actions>
          <v-btn text @click="closeModal">Abbrechen</v-btn>
          <v-btn
              text
              @click="saveTodo"
...
export default {
  ...
  computed: {
    isFormValid() {
      ...
      const isDeadlineValid = this.dateRuleForOptionalField(this.newTodo.deadline) === true;

      return isTitleValid && isPriorityValid && (this.newTodo.deadline === '' || isDeadlineValid);
    }
  },
  methods: {
    ...
    saveTodo() {
      ...
      if (deadlineDate) {
        ...
        } else if (deadlineDate.getMonth() === currentDate.getMonth() && deadlineDate.getFullYear() === currentDate.getFullYear()) {
          this.nextTwoWeeks.push({ text: this.newTodo.title, priority: this.newTodo.priority });
          this.nextTwoWeeks = this.sortTodos(this.nextTwoWeeks);
        } else {
          this.longerDeadlines.push({ text: this.newTodo.title, priority: this.newTodo.priority });
    ...
    sortTodos(todos) {
      return todos.sort((a, b) => {
        ...
        return priorityComparison !== 0 ? priorityComparison : a.text.localeCompare(b.text);
      });
    }
  }
}
